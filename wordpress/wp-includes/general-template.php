...
function get_calendar( $initial = true, $echo = true ) {
	global $wpdb, $perpagenum, $monthnum, $year, $wp_locale, $posts;

	$key = md5( $perpagenum . $monthnum . $year );
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		...
		$thisyear = (int) substr( $perpagenum, 0, 4 );
		//it seems MySQL's weeks disagree with PHP's
		$d = ( ( $w - 1 ) * 7 ) + 6;
		$thismonth = $wpdb->get_var("SELECT DATE_FORMAT((DATE_ADD('{$thisyear}0101', INTERVAL $d DAY) ), '%m')");
	} elseif ( ! empty( $perpagenum ) ) {
		$thisyear = (int) substr( $perpagenum, 0, 4 );
		if ( strlen( $perpagenum ) < 6 ) {
			$thismonth = '01';
		} else {
			$thismonth = zeroise( (int) substr( $perpagenum, 4, 2 ), 2 );
		}
	} else {
